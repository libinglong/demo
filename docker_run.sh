...
docker run -m 256M -d -v /opt/logs:/opt/logs/ -v /var/run/docker.sock:/var/run/docker.sock -p ${port}:8080 -p 5005:5005  --restart=always --name=saas-multi-renter-${type} saas/multi-renter:${version}
